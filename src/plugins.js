	...
	plugins = {
		...
		init: function() {
			...
			this.reload(function(err) {
				if (err) {
					if (global.env === 'development') winston.info('[plugins] discuss encountered a problem while loading plugins', err.message);
					return;
				}
		...
		registerHook: function(id, data, callback) {
			/*
				`data` is an object consisting of (* is required):
					`data.hook`*, the name of the discuss hook
				...
				function(next) {
					fs.readdir(npmPluginPath, function(err, dirs) {
						dirs = dirs.map(function(file) {
							...
							if (stats.isDirectory() && file.substr(npmPluginPath.length + 1, 14) === 'discuss-plugin-') return true;
							else return false;
						});
