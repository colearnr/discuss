var RDB = require('./redis.js'),
	...
	notifications = {
		...
		get: function(nid, uid, callback) {
			logger.log('debug', 'Notifications get', nid, uid);
			RDB.multi()
		...
		create: function(text, path, uniqueId, callback) {
			logger.log('debug', 'Notifications create', text, path, uniqueId);
			/**
		...
		push: function(nid, uids, callback) {
			logger.log('debug', 'Notifications push', nid, uids);
		...
		remove_by_uniqueId: function(uniqueId, uid, callback) {
			logger.log('debug', 'Notifications remove by uniqueId', uniqueId, uid);
			async.parallel([
				function(next) {
		...
		mark_read: function(nid, uid, callback) {
			logger.log('debug', 'Notifications mark read', nid, uid);
			if (uid) {
		...
		mark_read_multiple: function(nids, uid, callback) {
			logger.log('debug', 'Notifications mark read multiple', nids, uid);
		...
		mark_all_read: function(uid, callback) {
			logger.log('debug', 'Notifications mark all read', uid);
