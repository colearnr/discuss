...
(function (Meta) {
			...
			fs.readdir(themePath, function (err, files) {
				async.filter(files, function (file, next) {
					fs.stat(path.join(themePath, file), function (err, fileStat) {
						...
						next((fileStat.isDirectory() && file.slice(0, 13) === 'discuss-theme-'));
					});
				}, function (themes) {
	...
	Meta.title = {
		build: function (urlFragment, current_user, callback) {
			...
			async.parallel({
				...
				if (err) {
					title = Meta.config.title || 'discuss';
				} else {
					title = (values.title ? values.title + ' | ' : '') + (Meta.config.title || 'discuss');
				}
	...
	Meta.js = {
		...
		minFile: path.join(__dirname, '..', 'public/src/discuss.min.js'),
		get: function (callback) {
			var mtime,
		...
		minify: function (callback) {
			...
			fs.writeFile(Meta.js.minFile, minified.code, function (err) {
				if (!err) {
					...
					winston.error('Problem minifying client-side libraries.');
				}
			});
		}
	};
