...
<p class="lead">
	Thanks for choosing to install discuss! We&apos;ll need some information to set up your installation
	configuration...
</p>
<p>
	Please enter the details of your Redis server here. If redis is hosted on the same
	server as discuss, you can leave the default values as-is.
</p>
				...
				socket.once('api:config.redisTest', function(data) {
					if (data && data.status === 'ok') {
						...
						app.alert({
							...
							title: 'Setup Ready!',
							message:	'discuss is ready to continue with the setup process. ' +
