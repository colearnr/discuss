...
(function () {
	...
	app.loadConfig = function() {
		$.ajax({
			url: SOCKET_SERVER + RELATIVE_PATH + '/api/config',
			success: function (data) {
				...
				if(socket) {
					...
					socket = io.connect(SOCKET_SERVER + RELATIVE_PATH, {
	...
	app.logout = function() {
		$.post(SOCKET_SERVER + RELATIVE_PATH + '/logout', {
			_csrf: $('#csrf_token').val()
		}, function() {
			window.location = SOCKET_SERVER + RELATIVE_PATH + '/';
		});
	}
	...
	app.createNewPosts = function (data) {
		...
		if (data && data.posts && data.posts[0]) {
			data.posts[0].display_moderator_tools = (data.posts[0].uid == app.uid) ? 'show' : 'none';
			pid = data.posts[0].pid;
		}
		if (pid && $('#post_anchor_' + pid).length) {
			return;
		}
	...
	jQuery('document').ready(function () {
		$('#search-form').on('submit', function (e) {
			...
			if(search_term) {
				ajaxify.go("search/" + search_term, null, "search");
				input.val('');
				return false;
			}
		});
