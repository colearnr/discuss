(function (module) {
		...
		function loadServer() {
			...
			function getTemplates(directory) {
				for (var t in templatesToLoad) {
					(function (file) {
						var tmpFileName = directory + '/' + file + '.tpl';
						fs.readFile(tmpFileName, function (err, html) {
	...
	templates.load_template = function (callback, url, template) {
		...
		jQuery.get(RELATIVE_PATH + '/api/' + api_url, function (data) {
			...
			app.alertError("Looks like you are disconnected. Trying to reconnect ...");
		});
		...
		function parse_template() {
			...
			if (typeof global !== "undefined") {
				...
				template_data['socket_server'] = nconf.get('socket_server');
			}
			else {
				template_data['relative_path'] = RELATIVE_PATH;
				template_data['socket_server'] = SOCKET_SERVER;
			}

			translator.translate(templates[tpl_url].parse(template_data), function (translatedTemplate) {
				// Fix embed not working in https mode
				if (APP_HOME_HTTP && APP_HOME_HTTPS && typeof translatedTemplate == "string") {
					translatedTemplate = translatedTemplate.replace(new RegExp(APP_HOME_HTTP, 'ig'), APP_HOME_HTTPS);
				}
