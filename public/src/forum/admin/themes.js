define(function() {
	...
	Themes.init = function() {
						...
						case 'use':
							...
							socket.emit('api:admin.theme.set', {
								...
								app.alert({
									...
									type: 'success',
									title: 'Theme Changed',
									message: 'You have successfully changed your discuss\'s theme. Please restart to see the changes.',
									timeout: 2500
								});
							});
						break;
		...
		revertEl.addEventListener('click', function() {
			bootbox.confirm('Are you sure you wish to remove the custom theme and restore the discuss default theme?', function(confirm) {
				if (confirm) {
					socket.emit('api:admin.theme.set', {
						type: 'local',
						id: 'discuss-theme-cerulean'
					}, function(err) {
						app.alert({
							alert_id: 'admin:theme',
							type: 'success',
							title: 'Theme Changed',
							message: 'You have successfully reverted your discuss back to it\'s default theme. Please restart to see the changes.',
							timeout: 3500
						});
					});
				}
