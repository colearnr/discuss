define(function() {
	...
	Topic.init = function() {
		var expose_tools = templates.get('expose_tools'),
			...
			short_url = templates.get('short_url');
						...
						socket.once('api:categories.get', function(data) {
							...
							commitEl.addEventListener('click', function() {
								if (!commitEl.disabled && targetCid) {
									...
									commitEl.innerHTML = 'Moving <i class="fa fa-spin fa-refresh"></i>';
		...
		function reply_fn() {
			...
			if (selection && $(selection.baseNode).parents('.post-content').length > 0) {
				var snippet = selection.toString();
				if (snippet.length > 0) {
					selectionText = '<blockquote>' + snippet.replace(/\n/g, '<br/>') + '</blockquote>';
				}
			}

			if (thread_state.locked !== '1') {
				require(['composer'], function(cmp) {
					cmp.push(tid, null, null, selectionText.length > 0 ? selectionText + '<br/>' : '');
				});
			}
		};
		$('#post-container').on('click', '.post_reply', reply_fn);
		$('#post_reply').on('click', reply_fn);
		var quote_fn = function() {
			if (thread_state.locked !== '1') {
				var pid = $(this).parents('li').attr('data-pid');
				socket.once('api:posts.getRawPost', function(data) {
					quoted = '<blockquote>' + data.post.replace(/\n/g, '<br/>') + '</blockquote><br/>';
					require(['composer'], function(cmp) {
		...
		$('#post-container').on('click', '.quote', quote_fn);
		...
		$('#post-container').on('click', '.link', function() {
			var pid = $(this).parents('li').attr('data-pid');
			$('#post_' + pid + '_link').val(short_url).stop(true, false).fadeIn().select().focus();
			...
			if(data) {
				...
				function createUserIcon(uid, picture, userslug, username) {
					...
					if(!activeEl.find("[href='/user/"+ data.users[i].userslug + "']").length) {
						...
						var userUrl = (userslug && userslug != 'guest') ? '<a href="/user/' + userslug + '"></a>' : '#';
						var userLink = $(userUrl).append(userIcon);
				...
				if(anonymousCount || remainingUsers) {
					...
					var anonLink = $('<i class="fa fa-user anonymous-box"></i>');
		...
		function set_locked_state(locked, alert) {
			...
			if (locked === true) {
				lockThreadEl.innerHTML = '<i class="fa fa-unlock"></i> Unlock Thread';
				threadReplyBtn.disabled = true;
				threadReplyBtn.innerHTML = 'Locked <i class="fa fa-lock"></i>';
				...
				lockThreadEl.innerHTML = '<i class="fa fa-lock"></i> Lock Thread';
		...
		function set_delete_state(deleted) {
			...
			if (deleted) {
				deleteTextEl.innerHTML = '<i class="fa fa-comment"></i> Restore Thread';
				...
				deleteTextEl.innerHTML = '<i class="fa fa-trash-o"></i> Delete Thread';
		...
		function set_pinned_state(pinned, alert) {
			...
			if (pinned) {
				pinEl.innerHTML = '<i class="fa fa-thumb-tack"></i> Unpin Thread';
				if (alert) {
					app.alert({
						'alert_id': 'thread_pin',
						type: 'success',
				...
				pinEl.innerHTML = '<i class="fa fa-thumb-tack"></i> Pin Thread';
				if (alert) {
					app.alert({
						'alert_id': 'thread_pin',
						type: 'success',
